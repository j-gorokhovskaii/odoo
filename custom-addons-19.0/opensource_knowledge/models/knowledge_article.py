...
class KnowledgeArticle(models.Model):
    ...
    def write(self, vals):
        ...
        # 19.0: notify when an article transitions to "published"
        # (replaces the old base.automation record, removed during the 18->19 port)
        newly_published = self.env['knowledge.article']
        if vals.get('stage') == 'published':
            newly_published = self.filtered(lambda a: a.stage != 'published')
        res = super().write(vals)
        for article in newly_published:
            article.message_post(
                body=_("Article has been published and is now available to users."),
                subject=_("Article Published"),
